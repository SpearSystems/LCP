...
**Status:** v1.0.1 is the published, stable protocol and reference implementation release. The signed `v1.0.1` tag resolves to commit `61886511d2c9424ffb197d0788b049216a4645a2`, and the GitHub release includes the signed manifest, SBOM, provenance, and package evidence. The current tree is the prepared v1.0.2 candidate: it carries the repository-audit remediation (attachment authorization, lifecycle enforcement, ten-SDK validator equivalence, privacy ping policy, MCP hardening, and more) plus the Kotlin SDK compile fix, dependency pins, and benchmark fixes; its conformance runner passes 30/30 test vectors. The repository includes maintained Tier 1 Python, TypeScript, Go, and C# SDKs; reference Java, PHP, Rust, Ruby, Kotlin, and Swift SDKs; a production-oriented reference platform/router; an optional MCP adapter; and a Docker sandbox using the same platform code path. See [docs/RELEASE.md](docs/RELEASE.md) for verification instructions.
...
Postgres integration, indexed-offer performance, dependency/security controls,
container scan, SBOMs, and Kubernetes admission fixtures. The non-publishing
Performance workflow runs synthetic candidate-reduction and PostgreSQL 18.4
scale checks with thresholds and downloadable JSON/log reports. The live Kyverno admission job builds its own
...
| Performance | [Performance](https://github.com/SpearSystems/LCP/actions/workflows/performance.yml) | Indexed-offer reduction and synthetic PostgreSQL 18.4 intake/delivery thresholds; never publishes artifacts |
...
- [Consent and evidence guide](docs/CONSENT-GUIDE.md)
...
- [v1.0.2 release record](docs/RELEASE-TICKET-1.0.2.md)
