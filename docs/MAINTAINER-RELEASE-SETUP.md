...
Central's OSSRH compatibility endpoint. The Kotlin publication must include
sources, Javadocs, detached PGP signatures, SCM metadata, and developer
metadata before Central validation. The tagged release workflow must then
...
Central Publisher Portal. If validation closes a compatibility repository,
drop that failed repository before retrying. Java uses the Central publishing
Maven plugin directly.
