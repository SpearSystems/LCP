...
| Java | `com.networknt:json-schema-validator` 1.5.9 → 3.0.6 | The 3.x line is a full rewrite on Jackson 3 (`tools.jackson`). The validator was migrated from `JsonSchemaFactory`/`SpecVersion` to `SchemaRegistry`/`Schema`/`SpecificationVersion`, and the SDK's own JSON parsing moved from `com.fasterxml.jackson` to `tools.jackson` 3.1.5 (fixes CVE-2026-54512, CVE-2026-54513, and CVE-2026-59889). The Jackson 2 pins that mitigated the 1.5.9 transitive CVEs are removed. Verified with `mvn test`; Dependabot PR #16. |
| Java | `tools.jackson.core:jackson-databind` 3.1.4 → 3.1.5 | Follow-up to the networknt 3.x migration: osv-scanner flagged 3.1.4 for CVE-2026-59889 (GHSA-5gvw-p9qm-jgwh, `@JsonView` bypass for `@JsonUnwrapped` container properties on deserialization). Bumped the explicit pin to the patched 3.1.5 in both the Java pom and the Kotlin build; both dependency trees resolve 3.1.5 and osv-scanner reports zero findings. |
| Kotlin | `tools.jackson.core:jackson-databind` 3.1.4 → 3.1.5 | Same CVE-2026-59889 rationale as Java; verified with the Gradle test suite. |
