...
automation co-author identities (currently Codebuff) before Git creates the
commit. It preserves legitimate human `Co-Authored-By` trailers, the rest of
the message, the author, and normal commit flow. The Attribution policy
workflow is informational: it reports commits created without the hook, but it
never rewrites pull-request, fork, or protected-branch history. Human
collaboration and AI assistance still belong in the pull request description,
where the contributor can explain and attest to the change.
