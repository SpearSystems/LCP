...
blocking a commit. Enable the tracked hook once per checkout:

```bash
git config core.hooksPath .githooks
```

The `commit-msg` hook removes `Generated with`/`Generated by` lines and
`Co-Authored-By` trailers before Git creates the commit, preserving the rest of
the message, author, and normal commit flow. The Attribution policy workflow
